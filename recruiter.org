...
** Explanation
   ...
   - We don't have contention when picking jobs because there's only one recruiter process
     so it doesn't have to lock anything, when a worker it's available it can suddenly
     became unavailable because the only one who can assign job is the recruiter. What can
     happen is that a worken can become available in the meantime but who cares, it will
     be considered in the next round of job assignment
   ...
   - Bulk updates are faster
     #+BEGIN_EXAMPLE
     TODO... bulk insert assigning jobs to workers
     #+END_EXAMPLE
